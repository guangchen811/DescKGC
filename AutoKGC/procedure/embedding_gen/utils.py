...
def tokenize_and_encode(
    sentences, tokenizer, model, max_batch_size=64, padding_length=128
):
    """
...
def cos_sim_between_sentences_sets(
    sentences_set1, sentences_set2, tokenizer, model
):
    # con
    ...
    for i, encoded_sen1 in enumerate(encoded_set1):
        for j, encoded_sen2 in enumerate(encoded_set2):
            similarity = cos(
                encoded_sen1.unsqueeze(0), encoded_sen2.unsqueeze(0)
            )
