...
class ChromaVectorStore:
    ...
    def get_from_specific_source(self, source):
        if source == "provided":
            ...
            raise ValueError(
                "embedding source should be either provided or generated"
            )
        return self.collection.get(
    ...
    def query_from_specific_source(
        self, source, query_texts, n_results, **kwargs
    ):
        if source == "provided":
            embedding_source = "summary"
        elif source == "generated":
    ...
    def query_from_specific_type(
        self, entity_type, query_texts, n_results, **kwargs
    ):
        results = self.collection.query(
            query_texts=query_texts,
            n_results=n_results,
