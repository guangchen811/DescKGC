...
def extract_entities_from_paper(paper_id_type, paper_id_value, topic, shortenings, vs_key_info, db_manager, extract_chain):
    ...
    assert all(key in entity_keys for key in [embedding_key, *metadata_keys]), "KeyError: some keys are not in the paper dict. Please check the vs_key_info in config.yaml"
