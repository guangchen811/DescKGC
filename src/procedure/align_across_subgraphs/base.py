...
if __name__ == '__main__':
    ...
    for i in range(len(entities_list)):
        for j in range(i, len(entities_list)):
            cos_metric = cos_metric_between_sent_sets(
                entities_list[i], entities_list[j], tokenizer, model)
            add_sim_metric_to_db(db_manager, cos_metric,
                                 entities_list[i], entities_list[j])
